...
function renderCalendar(date) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        calendarHTML += `<div class="text-center w-6 py-2 border rounded">${i}</div>`;
    }
...
function renderHabits() {
    ...
    habitsElement.innerHTML = habits
        ...
        <div class="p-4 bg-white rounded shadow">
            ...
            <div class="grid grid-flow-col gap-1.4 w-full">
                ...
                .map(
                    (_, i) => {
                        ...
                        return `<input type="checkbox" index="${index}" data-day="${day}"
                            ${isChecked ? 'checked' : ''} class="w-6 h-6 rounded border-gray-300">`;
                    }
                )
                .join('')}
            </div>
    ...
    habitElement.innerHTML = `
        <div class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
            <div class="bg-white p-6 rounded shadow-lg">
                ...
                <input type="text" id="newHabitName" class="border p-2 w-full mb-4" placeholder="Enter habit name">
...
habitsElement.addEventListener('change', (e) => {
    if (e.target.tagName === 'INPUT') {
        const habitIndex = e.target.getAttribute('index');
